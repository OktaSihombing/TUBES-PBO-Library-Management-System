...
class SampleApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.shared_data = {'SubID':tk.StringVar()}
        self.shared_data2 = {'tipe':tk.StringVar()}
    ...
    def __init__(self, parent, controller):
        ...
        def CekLogin():
            ...
            if row==None:
                ...
                messagebox.showinfo("Success", "Login Success")
                global SubID, tipe
                cur.execute("select type from Subscribers where username=%s and password=%s", (username_entry_box.get(), password_entry_box.get()))
                tp=cur.fetchall()
                for z in tp:
                    z[0]
                tipe=z[0]
                cur.execute("select subscriber_id from Subscribers where username=%s and password=%s", (username_entry_box.get(), password_entry_box.get()))
                si=cur.fetchall()
                for x in si:
                    x[0]
                SubID=x[0]
                controller.shared_data2['tipe'].set(tipe)
                controller.shared_data['SubID'].set(SubID)
    ...
    def __init__(self, parent, controller):
        ...
        def CekSigninAdmin():
            ...
            try:
                ...
                messagebox.showerror("Error","Cannot Sign In")
    ...
    def __init__(self, parent, controller):
        ...
        def CekSigninStudent():
            ...
            try:
                ...
                messagebox.showerror("Error","Cannot Sign")
    ...
    def __init__(self, parent, controller):
        ...
        def InsertBook():
            ...
            try:
                ...
                messagebox.showerror("Error","Cannot Insert Book")
...
class Lending(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        cur.execute("select * from borrowing order by Subscriber_id;")
        my_tree = ttk.Treeview(self)
        my_tree['show']='headings'
        ...
        my_tree.column("Subscriber_ID", width=150, minwidth=50, anchor=CENTER)
...
class BorrowBook(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        cur.execute("select Nama, Item_ID, category, title, author from items join library where library.id=items.library_id;")
        my_tree = ttk.Treeview(self)
        my_tree['show']='headings'
        ...
        my_tree["columns"]=("Nama","Item_ID","Category", "Title", "Author")
        my_tree.column("Nama", width=200, minwidth=100, anchor=CENTER)
        my_tree.column("Item_ID", width=70, minwidth=100, anchor=CENTER)
        my_tree.column("Category", width=100, minwidth=150, anchor=CENTER)
        my_tree.column("Title", width=250, minwidth=100, anchor=CENTER)
        my_tree.column("Author", width=200, minwidth=150, anchor=CENTER)

        my_tree.heading("Nama", text="Library", anchor=CENTER)
        ...
        l1=tk.Label(f, text="Item ID", width=8, font=('roboto',12, "bold"), background="#cb945c")
        l1.place(x=70,y=110)
        e1=tk.Entry(f,width=25,textvariable=t1)
        e1.place(x=230,y=110)

        l2=tk.Label(f, text="Category", width=8, font=('roboto',12, "bold"), background="#cb945c")
        l2.place(x=70,y=150)
        e2=tk.Entry(f,width=25,textvariable=t2)
        e2.place(x=230,y=150)

        l3=tk.Label(f, text="Title", width=8, font=('roboto',12, "bold"), background="#cb945c")
        l3.place(x=70,y=190)
        e3=tk.Entry(f,width=25,textvariable=t3)
        e3.place(x=230,y=190)

        l4=tk.Label(f, text="Author", width=8, font=('roboto',12, "bold"), background="#cb945c")
        l4.place(x=70,y=230)
        e4=tk.Entry(f,width=25,textvariable=t4)
        ...
        def Borrow():
            ...
            t=tipe
            if t == "Golden":
                newtime=oldtime+relativedelta(months=+3)
            elif t=="Regular":
            ...
            sqlb = "insert into borrowingsubscribers (subscriber_id,Title_Book, Borrow_Date, Item_ID, Return_Date) values (%s, %s, %s, %s, %s)"
            try:
                val=(yt,oldtime,iid,newtime)
                cur.execute(sqla, val)
                con.commit()
                al=(yt,title,oldtime,iid,newtime)
                cur.execute(sqlb, al)
                con.commit()
                messagebox.showinfo("Success", "Borrow Book Success")
            except:
                messagebox.showerror("Error","Cannot Borrow Book")
        borrowbutton = tk.Button(self, text="Borrow",command=Borrow)
        borrowbutton.configure(font=('roboto',12, "bold"), bg='red', fg='white')
        borrowbutton.place(x=670, y=650)
...
class ReturnBook(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def returning():
            global SubID
            yt=SubID
            cur.execute("select Title_Book, Borrow_Date, Item_ID, Return_Date from BorrowingSubscribers where subscriber_id = %s;",(yt))
            my_tree = ttk.Treeview(self)
            my_tree['show']='headings'
        ...
        tampil_button = tk.Button(self,text="Tampilkan", command=returning,border=0)
        tampil_button.configure(font=('roboto',12, "bold"), bg='green', fg='white')
        tampil_button.pack()
